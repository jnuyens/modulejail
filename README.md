...
```sh
curl -fsSL https://raw.githubusercontent.com/jnuyens/modulejail/v1.6.2/modulejail | sudo sh
```
...
>
> ```sh
> curl -fsSL https://raw.githubusercontent.com/jnuyens/modulejail/v1.6.2/modulejail | sudo sh -s -- -p desktop
> ```
>
...
```sh
curl -fsSL https://raw.githubusercontent.com/jnuyens/modulejail/v1.6.2/modulejail | sudo sh -s -- -o /etc/modprobe.d/site-blacklist.conf
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/jnuyens/modulejail/v1.6.2/modulejail -o /tmp/modulejail
less /tmp/modulejail
...
```sh
# Debian / Ubuntu:
curl -fsSLO https://github.com/jnuyens/modulejail/releases/download/v1.6.2/modulejail_1.6.2_all.deb
sudo dpkg -i modulejail_1.6.2_all.deb

# RHEL / Fedora / Rocky:
curl -fsSLO https://github.com/jnuyens/modulejail/releases/download/v1.6.2/modulejail-1.6.2-1.noarch.rpm
sudo rpm -i modulejail-1.6.2-1.noarch.rpm
```
...
> ```
>
> From then on, `git tag -a v1.6.2 -m "..."` auto-signs without the
...
| `--uninstall-initramfs-hook` | Remove every known strip-hook file path regardless of which builder is currently detected (handles distro migrations), including the mkinitcpio `HOOKS` drop-in and the legacy pacman trigger. Prints the rebuild command afterwards. Requires root (since v1.4) |
...
- **`master`** tracks the latest stable release (currently `v1.6.2`).
